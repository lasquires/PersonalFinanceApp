...
import { items, plaid, plaidCode } from '@/lib/server/plaid';
export async function POST(request:Request) {
 ...
 }catch(error){const message=error instanceof Error?error.message:'';const auth=/required|Forbidden/.test(message);if(!auth){const details=(error as {response?:{data?:{error_message?:string;request_id?:string}}}).response?.data;console.error('Plaid link token failed',{code:plaidCode(error),...(details?.error_message?{message:details.error_message}:{}),...(details?.request_id?{requestId:details.request_id}:{})});}return NextResponse.json({error:auth?'Sign in with a household account.':'Account connection is not ready. Check the server setup and Plaid plan.'},{status:auth?401:503});}
}
